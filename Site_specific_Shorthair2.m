function [D, TEMP, H2O, rho_fix, v_wind] = Site_specific_Shorthair2(HEADER, D, Press, R_mol, Tc, a)
...
%consider additional pressure filtering
%focus = time > 605;
%bad = focus & D(110,:) < 55;
%D(110,bad) = NaN;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
h2o_hmp(1,missing) = mol_frac_goes(1,missing);
h2o_hmp = h2o_hmp *1000; %mmol h2o/ mol of wet air 

%----------------------------------------------------------------------
%irga h2o concentrations during this period
%----------------------------------------------------------------------
%h2o adjustment
...
%NOTE: need to get the fast flux filter h2o filter at 30 mol/mol of wet air
%--> This filter is set by the instrument response ==> We are currently
%allowing saturated h2o concentrations to be evaluated in our covariances &
%this should not be happening!
bad_H2Oc_mat = bad_H2Oc_mat | D(86, :) > 29.8; %This should have been done in fast but the filter was too loose - 

D(42, bad_H2Oc_mat) = NaN;

%convert to mixing ratio
%This is converted to a mixing ratio in the fastflux program
%irga h2o adjustment happened in the fast flux program
...
%----------------------------------------------------------------------
%adjust irga h2o to hmp during focus periods - nonlinear
focus = focus_1 & isfinite(h2o_hmp(1,:)) & isfinite(D(86,:));
...
%correct the h2o concentration
h2o_adj = (p(1,1) * D(86,:) .* D(86,:)) + (p(1,2) * D(86,:)) + p(1,3);
D(86, focus) = h2o_adj(1, focus);

%----------------------------------------------------------------------
%adjust irga h2o to hmp during focus periods
focus = focus_2 & isfinite(h2o_hmp(1,:)) & isfinite(D(86,:));
p = polyfit(D(86,focus), h2o_hmp(1,focus), 2); %second order polynomial
dh2o_hmp_dirga = polyder(p);%find the derivative
...
bad_H2Oc_goes = bad_H2Oc_goes | D(212, :) < -200; 
D(212, bad_H2Oc_goes) = NaN;

%----------------------------------------------------------------------
%adjust irga h2o to hmp during focus periods
focus = focus_1 & isfinite(h2o_hmp(1,:)) & isfinite(D(212,:));
...
%correct the h2o concentration
h2o_adj = (p(1,1) * D(212,:) .* D(212,:)) + (p(1,2) * D(212,:)) + p(1,3);
D(212, focus) = h2o_adj(1, focus);

%----------------------------------------------------------------------
%adjust irga h2o to hmp during focus periods
focus = focus_2 & isfinite(h2o_hmp(1,:)) & isfinite(D(212,:));
p = polyfit(D(212,focus), h2o_hmp(1,focus), 2); %second order polynomial
dh2o_hmp_dirga = polyder(p);%find the derivative
...
%~34% of the observations will be adjusted
%----------------------------------------------------------------------
%winds
%----------------------------------------------------------------------
%component wind problems - variance = 0 is impossible
%u_wind
u_mat_var=D(9,:);
...
for i=2:length_Cnumsince2009
    %proc data
    good_mat = time(1,:) < Cnumsince2009(i)-2 & time(1,:) > Cnumsince2009(i-1)+2 & isfinite(D(42,:))==1 & isfinite(D(100,:))==1 & focus_1(1,:)==0 & focus_2(1,:)==0;
    irga=D(42,good_mat);
    hmp=D(100,good_mat);
    p = polyfit(hmp,irga,1) %p(1) is slope; p(2) is intercept
    c_tm = time(1,:) <= Cnumsince2009(i) & time(1,:) > Cnumsince2009(i-1) & focus_1(1,:)==0 & focus_2(1,:)==0; %adjustment period
    ...
    %data logger
    good_dl = time(1,:) < Cnumsince2009(i)-2 & time(1,:) > Cnumsince2009(i-1)+2 & isfinite(D(86,:))==1 & isfinite(D(100,:))==1 & focus_1(1,:)==0 & focus_2(1,:)==0;
    irga=D(86,good_dl);
    hmp=D(100,good_dl);
    p = polyfit(hmp,irga,1) %p(1) is slope; p(2) is intercept
    c_tm = time(1,:) <= Cnumsince2009(i) & time(1,:) > Cnumsince2009(i-1) & focus_1(1,:)==0 & focus_2(1,:)==0; %adjustment period
    ...
    %goes
    good_goes = time(1,:) < Cnumsince2009(i)-2 & time(1,:) > Cnumsince2009(i-1)+2 & isfinite(D(212,:))==1 & isfinite(D(226,:))==1 & focus_1(1,:)==0 & focus_2(1,:)==0;
