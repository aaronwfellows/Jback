function combined_Working_3(sites2Proc, saveon, combinInfo)

%consider additional filter  - the D(145,:) filter - pump on
...
for iSite = sites2Proc
    ...
    elseif iSite ==7
        [D, T_consensus, h2o_consensus, need_rho_fix, uvw] = Site_specific_P301_2(HEADER, D, Press, R_mol, Tc, 1);
    elseif iSite ==8
        [D, T_consensus, h2o_consensus, need_rho_fix, uvw] = Site_specific_SJER2(HEADER, D, Press, R_mol, Tc, 1);
    elseif iSite ==9
        [D, T_consensus, h2o_consensus, need_rho_fix, uvw] = Site_specific_Shorthair2(HEADER, D, Press, R_mol, Tc, 1);
    elseif iSite ==10
    ...
    %% CO2 fluxes
    %----------------------------------------------------------------
    %UNITS: mol dry air /m^3 * m/s * micromol CO2/ mol dry air ==> micromol CO2 / m^2 / s
    
    %dry air density adjustment to mat proc
    D(46,need_rho_fix) = densityfix(1,need_rho_fix).*D(46,need_rho_fix);
    
    %UNITS: mol dry air /m^3 * m/s * micromol CO2/ mol moist air ==> micromol CO2 / m^2 / s
    %datalogger
    ...
    %% H2O fluxes
    %----------------------------------------------------------------
    %UNITS: mol dry air /m^3 * m/s * mmol h2o/ mol dry air ==> mmol h2o / m^2 / s
    
    %dry air density adjustment to mat proc
    D(47,need_rho_fix) = densityfix(1,need_rho_fix).*D(47,need_rho_fix);
    
    %data logger - not web corrected 
    ...
    %% Latent Heat
    %----------------------------------------------------------------
    %LE final calculation uses the best combined Fh2o in the homogenize and
    %combined section - This is for checking
    ...
    %This Ts should be in Celcius
    Lv   = (2.501-0.00237.*(T_consensus))*10^6; %units: J/kg
    
    %LOGGER
    lh_dl = 1e-6 * 18 .*Lv .* D(347,:); 
    ...
    elseif iSite ==7
        [D, T_consensus, h2o_consensus, need_rho_fix, uvw] = Site_specific_P301_2(HEADER, D, Press, R_mol, Tc, 2);
    elseif iSite ==8
        [D, T_consensus, h2o_consensus, need_rho_fix, uvw] = Site_specific_SJER2(HEADER, D, Press, R_mol, Tc, 2);
    elseif iSite ==9
        [D, T_consensus, h2o_consensus, need_rho_fix, uvw] = Site_specific_Shorthair2(HEADER, D, Press, R_mol, Tc, 2);
    elseif iSite ==10
    ...
    %Do things look OK?
    if combinInfo ==1
        [N_obs, headerN_obs, D_obs_out] = combinedinfo2(siteName, D, D_obs_out, N_obs, 2);
        disp('P301 and Shorthair combined breaks these data into parts')
        disp('These figures lump all data together, the combined uses only turbulent observations for Fh2o, and fco2')
    end
    
    %----------------------------------------------------------------------
    %The tower tilts at shorthair ==> we will combined before and after the
    %tip to homogenize correctly
    %-->  The double rotation used during the fast processing should remove
    %     the tip effects in the fast data.  ==> the regression should
   
    do_combined_here = 1; % do the combined in shorthairCombined
    
    if iSite == 7
        [D, HMERGE, Cleaned_D, DataSource, Cleaned_Header] = P301Combined(D, HMERGE, Cleaned_D, DataSource, Cleaned_Header);
        do_combined_here = 0; % we did the combined in the P301Combined
    end
    
    if iSite == 9
        [D, HMERGE, Cleaned_D, DataSource, Cleaned_Header] = shorthairCombined(D, HMERGE, Cleaned_D, DataSource, Cleaned_Header);
        do_combined_here = 0; % we did the combined in the shorthairCombined
    end
    %----------------------------------------------------------------------
    
    if do_combined_here == 1;
    %----------------------------------------------------------------------
    %1.Time
    %----------------------------------------------------------------------
    %Fill Data
    ...
    copyfile('combined_Working_3.m',['./version_archive/' siteName '_' date '_combined_Working_3.m'],'f');
end