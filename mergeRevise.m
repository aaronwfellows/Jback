...
for iSite=sites2Proc
    ...
    if exist(fin_goes, 'file') 
        ...
        if iSite == 7
            EXPDAY_GOES(EXPDAY_GOES < 471) = EXPDAY_GOES(EXPDAY_GOES < 471)+1;
            EXPDAY_GOES(EXPDAY_GOES > 530 & EXPDAY_GOES < 676) = EXPDAY_GOES(EXPDAY_GOES > 530 & EXPDAY_GOES < 676)+6/24;
        %elseif iSite == 6
        %    EXPDAY_GOES(EXPDAY_GOES<=1269) = EXPDAY_GOES(EXPDAY_GOES<=1269)-1;
        end
    ...
    EXPDAY_PROC = D(2,:);
    if iSite == 7
            EXPDAY_PROC(EXPDAY_PROC < 471) = EXPDAY_PROC(EXPDAY_PROC < 471)+1;
            EXPDAY_PROC(EXPDAY_PROC > 530 & EXPDAY_PROC < 676) = EXPDAY_PROC(EXPDAY_PROC > 530 & EXPDAY_PROC < 676)+6/24;
            %% fix the month offset in the datalogger in June 2011
            bad = EXPDAY_PROC > 1221 & EXPDAY_PROC < 1225;
            EXPDAY_PROC(bad) = EXPDAY_PROC(bad) + 31;
    end
