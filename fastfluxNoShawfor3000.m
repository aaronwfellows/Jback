function fastfluxNoShawfor3000(Sites2Proc)
...
for iSite = Sites2Proc      %L1
    ...
    for ii=i2Proc
        ...
        [h,d]                 = concat3000([fastSiteDir 'ts_data' dirSep  ],[],[JDInterval(ii)-1/1440  JDInterval(ii)+procInt+1/1440 300]);
        [hflux,dflux]         = concat3000([fastSiteDir 'flux' dirSep     ],[],[JDInterval(ii)+procInt    JDInterval(ii)+procInt+1/24/3600 900]);
        [hvariance,dvariance] = concat3000([fastSiteDir 'variance' dirSep ],[],[JDInterval(ii)+procInt    JDInterval(ii)+procInt+1/24/3600 900]);
       
        if isempty(d)
            disp('ts data not being read.  Check concat3000 line in FSC program.')
            %pause;
        end
        if isempty(dvariance)
            disp('variance data not being read.  Check concat3000 line in FSC program.')
            %pause;
        end
        if isempty(hflux)
            disp('flux data not being read.  Check concat3000 line in FSC program.')
            %pause;
        end
        ...
        if ~isempty(d)
            ...
            if NGOOD_SONIC_AND_IRGA > SF*900   
                ...
                if TauandDelay == 1
                    ...
                    [CO2_,H2O_,RHOM_,FCO2_,FH2O_,HLATENT_,HSENSIBLE_,GAINS_, Ntemp_, Nco2_, Nh2o_]=FluxClosedPathNoShaw3000(UVWROT(3,:),Ts,SONDIAG,co2,h2o,IRGADIAG,PFlux,sitePress(iSite));
                    Tau_c_max_=NaN;
                    Tau_h2o_max_=NaN;
                end
