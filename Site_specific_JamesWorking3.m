...
IRGAchanged = [9,12,2006;1,17,2007;4,3,2007;2,28,2008;6,10,2008;3,31,2009;4,2,2010;8,18,2011;6,19,2012;12,15,2012];
...
badHMP_goes = badHMP_goes | time > 2410.979 & time < 2410.9794;
...
bad_Ts_goes = bad_Ts_goes | focus & D(210,:) ==0;
focus = time>1889 & time <1907.76;
bad_Ts_goes = bad_Ts_goes | focus & D(210,:) ==0;
focus = time> 1912 & time < 1923.5;
bad_Ts_goes = bad_Ts_goes | focus & D(210,:) ==0;
focus = time> 2454 & time < 2454.5;
bad_Ts_goes = bad_Ts_goes | focus & D(210,:) ==0;
...
%RH 
badRHdl = time > 736.895 & time < 736.9;
focus = time > 360 & time < 430;
badRHdl = badRHdl | focus & D(98,:) < 0.005;
focus = time > 700 & time < 1200;
badRHdl = badRHdl | focus & D(98,:) < 0.005;
focus = time > 1410 & time < 2700;
badRHdl = badRHdl | focus & D(98,:) < 0.005;
D(98,badRHdl)=NaN;

badRHgoes = time > 736.895 & time < 736.9;
focus = time > 360 & time < 425;
badRHgoes = badRHgoes | focus & D(224,:) < 0.005;
focus = time > 750 & time < 770;
badRHgoes = badRHgoes | focus & D(224,:) < 0.005;
focus = time > 860 & time < 920;
badRHgoes = badRHgoes | focus & D(224,:) < 0.005;
focus = time > 1070 & time < 1100;
badRHgoes = badRHgoes | focus & D(224,:) < 0.005;
focus = time > 1135 & time < 1150;
badRHgoes = badRHgoes | focus & D(224,:) < 0.005;
focus = time > 1400 & time < 1540;
badRHgoes = badRHgoes | focus & D(224,:) < 0.005;
focus = time > 1572 & time < 1578;
badRHgoes = badRHgoes | focus & D(224,:) < 0.005;
focus = time > 1605 & time < 1607;
badRHgoes = badRHgoes | focus & D(224,:) < 0.005;
focus = time > 1752 & time < 1758;
badRHgoes = badRHgoes | focus & D(224,:) < 0.005;
focus = time > 1774 & time < 1776;
badRHgoes = badRHgoes | focus & D(224,:) < 0.005;
focus = time > 1786 & time < 1789;
badRHgoes = badRHgoes | focus & D(224,:) < 0.005;
focus = time > 1800 & time < 1802;
badRHgoes = badRHgoes | focus & D(224,:) < 0.005;
focus = time > 1810 & time < 1890;
badRHgoes = badRHgoes | focus & D(224,:) < 0.005;
focus = time > 1910 & time < 2700;
badRHgoes = badRHgoes | focus & D(224,:) < 0.005;
D(224,badRHgoes)=NaN;
...
bad_H2Oc_mat = bad_H2Oc_mat |D(42,:) >30; %negative water vapor concentrations
...
bad_H2Oc_dl = bad_H2Oc_dl | D(86,:) > 30;
...
bad_goesCO2 = bad_goesCO2 | D(201,:) <=0;
bad_goesCO2 = bad_goesCO2 | D(261,:) > 0.01;  %poor irga performance 
...
for i=2:length_Cnumsince2006
    ...
    outrageous_p = isnan(p(1,1))==1 | isfinite(p(1,1)) == 0 | p(1,1) > 1.3 | p(1,1) < 0.8 | length(irga) < 30 | length(hmp) < 30
