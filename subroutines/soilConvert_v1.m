...
for iSite=sites2Proc
    ...
    for ii=1:length(soilFiles)
        ...
        try
            ...
            Soil_M = NaN * ones(4,length(soilData));
            Fuel_M = NaN * ones(2,length(soilData));
            Soil_T = NaN * ones(4,length(soilData));
            LWS    = NaN * ones(3,length(soilData)); 
            StartTemp_C  = NaN * ones(6,length(soilData));
            Temp_1sec_C  = NaN * ones(6,length(soilData));
            Temp_30sec_C = NaN * ones(6,length(soilData));
            DeltaT_C     = NaN * ones(6,length(soilData));
            box_T        = NaN * ones(1,length(soilData));
            airT         = NaN * ones(1,length(soilData));
            snowD        = NaN * ones(1,length(soilData));
