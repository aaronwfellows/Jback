function [N_obs, headerN_obs, D_obs_out] = combinedinfo2(siteName, D, D_obs_out, N_obs, a)
...
elseif a ==2
    ...
    reg = ~isn & turb ==1;
    P1=polyfit(D(30,reg),D(343,reg),1)
    B1 = robustfit(D(30,reg),D(344,reg))
    
    isn=isnan(D(30,:)) ==1 | isnan(D(344,:)) == 1;
    reg = ~isn & turb ==1;
    P2=polyfit(D(30,reg),D(344,reg),1)
    B2 = robustfit(D(30,reg),D(344,reg))
    ...
    reg = ~isn & turb ==1;
    P1=polyfit(D(47,reg),D(347,reg),1)
    B1 = robustfit(D(47,reg),D(347,reg))
    
    isn=isnan(D(47,:)) ==1 | isnan(D(348,:)) == 1;
    reg = ~isn & turb ==1;
    P2=polyfit(D(47,reg),D(348,reg),1)
    B2 = robustfit(D(47,reg),D(348,reg))
